    ...
    <section className="w-full py-20 md:py-28 px-4 sm:px-6 lg:px-8 bg-forest-800 relative overflow-hidden">
      ...
      <div className="relative max-w-4xl mx-auto text-center">
        <div className="inline-flex items-center gap-2 rounded-full bg-copper-500/20 border border-copper-500/30 px-4 py-2 text-sm font-medium text-copper-300 mb-8">
          ...
          85 orthophonistes nous font déjà confiance
        </div>

        <h2 className="font-display text-4xl md:text-5xl lg:text-6xl font-bold text-beige-100 mb-6 text-balance leading-tight">
          Prêt à prescrire{" "}
          <span className="text-copper-400 italic">la rééducation</span>{" "}
          respiratoire ?
        </h2>

        <p className="text-lg text-forest-300 mb-10 max-w-2xl mx-auto leading-relaxed">
          Essai 30 jours gratuit, sans carte bancaire. Annulable à tout moment.
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
          ...
          <Link
            href="/pricing"
            className="inline-flex items-center justify-center rounded-full border-2 border-forest-500 px-10 py-4 text-base font-semibold text-beige-300 hover:border-forest-300 hover:text-beige-100 transition-colors"
          >
            Voir les tarifs
          </Link>
        </div>
        ...
        <p className="text-sm text-forest-500">
          Sans CB · Sans engagement · RGPD · Hébergement France
        </p>
      </div>
    </section>
  );
